...
/*
request_desc(流程名称），folder_id（流程状态，1是进行中，0是已完成，其他的不是已取消就是未操作，可以归为同一类），create_user_id（发起人），create_date（发起时间）
*/
-- 改造查询
...
-- 需求  查询离职人员的基本操作
select * from USER_TABLE  where MI like'%朱庆%'--用姓名查出用户表里的信息
select * from USER_TABLE  where USER_ID ='u000270'--用工号查出用户表里的信息
select * from DEPARTMENTS  where DEPT_ID ='10540'--用部门ID查出部门信息
...
order by user_table.mi;
/*
需求 查询离职人发起的流程,通过某位经办人的形式
思路:先用简单查询查询 在使用连表完善信息
...
select sysdate from dual;查询当然时间,获得系统的默认格式
*/
select case ecl_tasks.status_id
--选择流程状态
when 1 then '待办' 
...
when 0 then '已完成'
      ...
      ecl_request_sheet.request_id 流程号, 
      ecl_request_sheet.request_desc 流程名称, 
      ecl_request_sheet.str_att_1 流程流水号, 
      to_char( ecl_request_sheet.create_date, 'yyyy-mm-dd') 创建时间, 
      to_char(ecl_tasks.create_date, 'yyyy-mm-dd') 审批起点, 
      ecl_request_sheet.create_user_id 流程发起人, 
      ecl_tasks.assignee_name 审批人, 
      user_table.user_id 审批人ID, 
      user_table.mi 审批人姓名 
from ecl_request_sheet --从流程查询表中查询
inner join ecl_tasks on ecl_tasks.request_id=ecl_request_sheet.request_id--通过流程号去连接任务流程表和任务查询表
inner join user_table on ecl_tasks.assignee = user_table.myws_id --通过审批人id和人员信息表连接
where  ecl_request_sheet.create_user_id ='u000560'and  ecl_request_sheet.create_date between '01-1月-15'and'31-12月-15'--选择创建者和时间段
order by ecl_tasks.create_date;--默认创建日期排序
